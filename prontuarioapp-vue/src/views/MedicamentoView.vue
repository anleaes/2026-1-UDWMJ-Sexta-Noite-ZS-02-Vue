  ...
  <div style="padding: 20px; font-family: sans-serif;">
    ...
    <div style="background-color: #f8f9fa; padding: 15px; border-radius: 4px; margin-bottom: 20px; border: 1px solid #e2e8f0;">
      ...
      <form @submit.prevent="salvarMedicamento" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px;">
        ...
        <input v-model="medForm.categoria" type="text" placeholder="Categoria (Ex: Analgésico)" style="padding: 8px; border: 1px solid #ccc; border-radius: 4px;" />
        <label style="display: flex; align-items: center; gap: 5px; font-size: 14px;">
          <input v-model="medForm.e_controlado" type="checkbox" /> Requer Tarja Preta / Controlado?
        </label>
        <label style="display: flex; align-items: center; gap: 5px; font-size: 14px;">
          <input v-model="medForm.tem_generico" type="checkbox" /> Possui equivalente Genérico?
        </label>
        <div style="display: flex; gap: 5px;">
          <button type="submit" style="flex: 1; background-color: #2ecc71; color: white; border: none; padding: 8px; border-radius: 4px; font-weight: bold; cursor: pointer;">Salvar</button>
    ...
    <div v-if="loading" style="color: #3498db;">🔄 Atualizando catálogo de medicamentos...</div>
    <table v-else border="1" cellpadding="10" style="width: 100%; border-collapse: collapse; background-color: white; text-align: left;">
      ...
      <tbody>
        <tr v-for="m in medicamentosFiltrados" :key="m.id">
          ...
          <td>{{ m.categoria }}</td>
          <td>
            <span :style="m.e_controlado ? 'color: #c0392b; font-weight: bold; background-color: #fadbd8; padding: 2px 6px; border-radius: 4px;' : 'color: #27ae60;'">
              {{ m.e_controlado ? '⚠️ Sim' : 'Não' }}
            </span>
          </td>
          <td>{{ m.tem_generico ? '🟢 Sim' : '❌ Não' }}</td>
          <td style="text-align: center;">
            <button @click="carregarParaEditar(m)" style="background-color: #f39c12; color: white; border: none; padding: 4px 6px; border-radius: 4px; cursor: pointer; margin-right: 4px;">✏️</button>
            <button @click="deletarMedicamento(m.id)" style="background-color: #e74c3c; color: white; border: none; padding: 4px 6px; border-radius: 4px; cursor: pointer;">🗑️</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  data() {
    return {
      medicamentos: [], filtro: '', loading: true, error: null, editandoId: null,
      medForm: { nome_referencia: '', principio_ativo: '', categoria: '', e_controlado: false, tem_generico: false }
    };
  },
  computed: {
    medicamentosFiltrados() {
      ...
      return this.medicamentos.filter(m => (m.nome_referencia || '').toLowerCase().includes(q) || (m.principio_ativo || '').toLowerCase().includes(q));
    }
  },
  ...
  methods: {
    ...
    async salvarMedicamento() {
      try {
        if (this.editandoId) {
          const response = await axios.put(`http://localhost:8000/medicamento/api/${this.editandoId}/`, this.medForm);
          const idx = this.medicamentos.findIndex(m => m.id === this.editandoId);
          this.medicamentos[idx] = response.data;
          this.editandoId = null;
        } else {
          const response = await axios.post('http://localhost:8000/medicamento/api/', this.medForm);
          this.medicamentos.push(response.data);
        }
        this.medForm = { nome_referencia: '', principio_ativo: '', categoria: '', e_controlado: false, tem_generico: false };
      } catch (err) { alert("Erro ao processar requisição."); }
    },
    carregarParaEditar(m) { this.editandoId = m.id; this.medForm = { ...m }; },
    cancelarEdicao() { this.editandoId = null; this.medForm = { nome_referencia: '', principio_ativo: '', categoria: '', e_controlado: false, tem_generico: false }; },
    async deletarMedicamento(id) {
      ...
      } catch (err) { alert("Bloqueado: Verifique se este item está vinculado a uma receita ativa."); }
    }
  }
};
</script>