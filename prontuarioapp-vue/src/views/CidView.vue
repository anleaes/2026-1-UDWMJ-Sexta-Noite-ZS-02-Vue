    ...
    <table v-else border="1" cellpadding="10" style="width: 100%; border-collapse: collapse; text-align: left; background-color: white;">
      ...
      <tbody>
        ...
        <tr v-if="filteredCids.length === 0">
          <td colspan="4" style="text-align: center; color: gray; padding: 15px;">Nenhum CID encontrado.</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  data() {
    return {
      searchQuery: '', cids: [], loading: true, error: null, editandoId: null,
      cidForm: { cod_cid: '', descricao: '' }
    };
  },
  computed: {
    filteredCids() {
  ...
  methods: {
    ...
    async salvarCid() {
      try {
        if (this.editandoId) {
          const response = await axios.put(`http://localhost:8000/cid/api/${this.editandoId}/`, this.cidForm);
          const idx = this.cids.findIndex(c => c.id === this.editandoId);
          this.cids[idx] = response.data;
          this.editandoId = null;
        } else {
          const response = await axios.post('http://localhost:8000/cid/api/', this.cidForm);
          this.cids.push(response.data);
        }
        this.cidForm = { cod_cid: '', descricao: '' };
      } catch (err) { alert("Erro ao salvar."); }
    },
    carregarParaEditar(c) { this.editandoId = c.id; this.cidForm = { cod_cid: c.cod_cid || c.codigo, descricao: c.descricao }; },
    cancelarEdicao() { this.editandoId = null; this.cidForm = { cod_cid: '', descricao: '' }; },
    async deletarCid(id) {
      ...
      } catch (err) { alert("Bloqueado: Código vinculado a prontuários ativos."); }
    }
  }
};
</script>